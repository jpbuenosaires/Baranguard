...
echo ============================================
echo  Baranguard AI Evaluation - Task 2/3: Pulling out names/contacts
echo ============================================
echo.
echo This tests how well the AI pulls the complainant, respondent, and
echo contact number out of a raw incident report. Nothing on this
echo computer is sent anywhere - everything runs locally, offline, same
echo as before.
echo.
...
set "HAS_PROGRESS="
if exist "fixtures\eval-incidents-v1.extraction.model.*.checkpoint.json" set "HAS_PROGRESS=1"
...
echo.
php scripts\ai-evaluate.php --task=extraction --engine=model --limit=1 --dry-run --verbose --resume
if errorlevel 1 (
    echo.
...
echo ============================================
echo Ready to run the full 350-record extraction test.
echo.
...
echo   evaluation-results-extraction-*.txt
echo   evaluation-log-extraction-*.txt
echo.
