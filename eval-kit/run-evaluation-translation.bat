...
echo (Tagalog) without dropping or inventing anything. Nothing on this
...
set "HAS_PROGRESS="
if exist "fixtures\eval-incidents-v1.translation.model.*.checkpoint.json" set "HAS_PROGRESS=1"
...
echo.
php scripts\ai-evaluate.php --task=translation --translate-to=fil --engine=model --limit=1 --dry-run --verbose --resume
if errorlevel 1 (
    echo.
...
echo ============================================
echo Ready to run the full 350-record translation test.
echo.
...
echo   evaluation-results-translation-*.txt
echo   evaluation-log-translation-*.txt
echo.
