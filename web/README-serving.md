...
It defaults to `http://127.0.0.1:8080/api/v1` (the API's PHP built-in
server option). If you're running the API through XAMPP's vhost instead
(`http://baranguard.local/api/v1`), or on a different port, edit that one
line in `index.html` to match.
