...
// 2026-09-27 (DEVLOG (38)): the old standalone W8 "AI Redaction Review"
// page is now the Redaction tab of the same case workspace
// blotter-detail.js renders — see that file's own doc for why. This
// file's tests moved from mounting `renderAiReviewPage` directly to
// mounting `renderBlotterDetailPage` and switching tabs first, same
// `openTab()`/`openSection()` idiom blotter-detail.test.mjs and
// settings.test.mjs already use for their own in-page sections.
// The tab bar only exists once the page has finished its initial load —
// pageSuite.mjs's generic loading/error-state tests call `openDataView`
// during states where there is genuinely no tab bar yet, so this is a
// no-op then rather than a failed click() assertion.
...
describePage({
  name: 'Incident detail (Redaction tab)',
  render: renderBlotterDetailPage,
  roles: ['secretary'],
  param: 901,
  heading: /^Incident Record — INC-2026-901/,
  expectText: ['RAW-NARRATIVE-901', 'Original Reported Narrative'],
  openDataView: openTab('Redaction'),
  // Not `polls: true` — incident 901's default draft fixture is already
  // `completed`, so no polling ever starts here (matches the original
  // standalone-page suite, which never asserted a stop()-clears-timers
  // guarantee either; would need a dedicated `status: 'processing'`
  // fixture override to test for real, out of scope for this rename).
});

describe('Redaction tab behaviour', () => {
  afterEach(() => cleanup());

  async function mountOnRedactionTab(param) {
    const ctx = mountPage(renderBlotterDetailPage, { role: 'secretary', param });
    await settle();
    await openTab('Redaction')(ctx);
    await settle();
    return ctx;
  }

  test('raw narrative and AI draft are shown side by side', async () => {
    await mountOnRedactionTab(901);
    const body = text();
  ...
  test('approval sends the exact draft_version and the draft text (Rule 4), then switches to the Blotter tab', async () => {
    ...
    // No page navigate() — switchTab('blotter') instead (2026-09-27).
    assert.equal(ctx.navigations.length, 0, 'approval must switch tabs, not navigate()');
    assert.match(text(ctx.root), /Finalize blotter entry|Official Blotter Summary/i, 'did not land on the Blotter tab');
  });
  ...
  test('no draft yet is an empty state with a way to run redaction', async () => {
    const ctx = await mountOnRedactionTab(904);
  ...
  test('Bikol translation is labelled as not yet validated (§2 Rule 16)', async () => {
    const ctx = await mountOnRedactionTab(903);
  ...
  test('the Lupon packet shortcut stays disabled until the blotter entry is finalized (server prerequisite)', async () => {
    const ctx = await mountOnRedactionTab(901);
    assert.equal(buttonByText(/lupon packet/i, ctx.root).disabled, true);
    cleanup();
    const ctx2 = await mountOnRedactionTab(903);
  ...
  test('the location line never falls back to a hard-coded barangay', async () => {
    ...
    const ctx = mountPage(renderBlotterDetailPage, { role: 'secretary', param: 777, userOverrides: { barangayId: 2 } });
    await settle();
    await openTab('Redaction')(ctx);
    await settle();
